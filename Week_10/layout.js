function layout(element){
    ...
    var style = getStyle(element);//获取预处理后的style对象
    ...
    if(style.flexDirection === 'row'){ //横向排列
        mainSize = 'width';
        mainStart = 'left';
        mainEnd = 'right';
        mainSign = +1;
    ...
    if(style.flexDirection === 'row-reverse'){ // 横向反向排列
        mainSize = 'width';
        mainStart = 'right';
        mainEnd = 'left';
        mainSign = -1;
    ...
    if(style.flexDirection === 'column'){//纵向排列
        mainSize = 'height';
        mainStart = 'top';
        mainEnd = 'bottom';
        mainSign = +1;
    ...
    if(style.flexDirection === 'column-reverse'){//纵向反向排列
        mainSize = 'height';
        mainStart = 'bottom';
        mainEnd = 'top';
        mainSign = -1;
    ...
    if(style.flexWrap === 'wrap-reverse'){//换行并在交叉轴方向倒置
   ...
   /*第二步 收集元素进行 */
    ...
    if(!style[mainSize]){
        style[mainSize] = 0;//父元素主轴尺寸初始化为0
        for(var i = 0;i< items.length;i++){
            var itemStyle =  getStyle(items[i]);
            if(itemStyle[mainSize] !== null || itemStyle[mainSize] !== (void 0)){//如果子元素的主轴方向设置了尺寸，父元素的主轴尺寸为子元素尺寸累加之和
    ...
    var mainSpace = style[mainSize];//主轴的空间（初始值为父元素的主轴尺寸）
    var crossSpace = 0;//交叉轴的空间
    for(var i = 0;i < items.length;i++){
        var item = items[i];
        itemStyle= getStyle(item);

        if(itemStyle[mainSize] === void 0){//如果这个子元素主轴方向没有设置尺寸，默认设置0
            itemStyle[mainSize] = 0;
        }

        if(itemStyle.flex){//如果子元素有flex属性，表示该元素可伸缩，则这个元素一定可以放进第一行，不管剩余多少空间
            flexLine.push(item);
        }else if(style.flexWrap === 'nowrap'){//如果设置了不换行，所有子元素放入第一行
            mainSpace -= itemStyle[mainSize];// 主轴剩余的空间减去当前放入第一行的元素主轴尺寸

            if(itemStyle[crossSize] !== void 0){//如果这个子元素交叉轴有设置尺寸
            ...
            if(mainSpace < itemStyle[mainSize]){ //如果主轴剩余空间尺寸小于当前子元素尺寸，说明这一行放不下这个子元素了
                ...
                mainSpace = style[mainSize];//重置存储主轴剩余空间的变量值
                crossSpace = 0;//重置存储交叉轴剩余空间的变量值
            }else{//能放下该元素
                flexLine.push(item);//向当前行放入子元素
            }
            
            if(itemStyle[crossSize] !== (void 0)){//如果子元素有设置交叉轴尺寸
    ...
    if(mainSpace < 0) {//如果计算出的主轴剩余空间是负的(只有一行的情况下会出现)
        //压缩每一项
        var scale = style[mainSize] / (style[mainSize] - mainSpace);//计算压缩比例(主轴尺寸  / 主轴方向期望的尺寸)
        ...
        flexLines.forEach(function(items){
            ...
            if(flexTotal > 0){//有设置了flex的元素，当前行一定会占满，用不到justify-content属性了
                var currentMain = mainBase;//元素的起始位置
                for(var i = 0;i<items.length;i++){
                    var item = items[i];
                    var itemStyle = getStyle(item);

                    if(itemStyle.flex){//按照flex在flexTotal中的占比，将剩余空间分配给flex元素的主轴尺寸
                        itemStyle[mainSize] = mainSpace * itemStyle.flex / flexTotal;
                    }
                    itemStyle[mainStart] = currentMain;//保存当前元素的起始位置
                    itemStyle[mainEnd] = itemStyle[mainStart] + mainSign * itemStyle[mainSize];//保存当前元素的结束位置
                    currentMain = itemStyle[mainEnd];//更新下一个元素的起始位置
                }
            }else{ //否则没有flex元素，根据justify-conotent属性来分配剩余空间
                /* 以row为例 */
                // currentMain ：排列的起始位置 step ：两个子元素之间的间隔
                if(style.justifyContent === 'flex-start'){//从左往右排
                    var currentMain = mainBase; //起始位置0
                    var step = 0;
                }

                if(style.justifyContent === 'flex-end'){//从右往左排（mainSign为-1，mainBase为容器宽度）
                    var currentMain = mainSpace * mainSign + mainBase;//容器width - 剩余空间
                    var step = 0;
                }

                if(style.justifyContent === 'center'){//居中对齐（mainSign为1，mainBase为0）
                    var currentMain = mainSpace / 2 * mainSign + mainBase; //起始位置(0) + 剩余空间的1/2
                    var step = 0;
                }
                if(style.justifyContent === 'space-between'){//两端对齐
                    var step = mainSpace / (items.length - 1) * mainSign;//总共有元素数量减1个间隔，平分即可
                    var currentMain = mainBase;//起始位置0
                }
    ...
    if(!style[crossSize]){ //如果没有设置交叉轴尺寸
        crossSpace = 0;
        style[crossSize] = 0;//初始为0
        for(var i = 0;i < flexLines.length;i++){
            style[crossSize] = style[crossSize] + flexLines[i].crossSpace;//累加上每一行的最大的元素的尺寸，最后得到总的交叉轴尺寸
        }
    }else{//设置了交叉轴尺寸

        crossSpace = style[crossSize];//交叉轴的剩余空间（初始为设置的交叉轴尺寸）
    ...
    // 有个疑惑，为什么不在前面的预处理阶段去设定
    if(style.flexWrap === 'wrap-reverse'){//换行反转的时候,交叉轴的起始位置为交叉轴末尾
    ...
    if(style.alignContent === 'flex-end'){//下对齐
        crossBase += crossSign * crossSpace; //起始位置设置为：初始位置 加上/减去 剩余空间
        step = 0;
    }
    if(style.alignContent === 'center'){//垂直居中
    ...
    flexLines.forEach(function(items){
        var lineCrossSize = style.alignContent === 'stretch' ? //align-content设置为strech，需要拉伸每一行
